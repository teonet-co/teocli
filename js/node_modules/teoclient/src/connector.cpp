...
size_t Connector::login(const char* host_name) {
...
NAN_METHOD(Connector::Login) {
	...
	Nan::ThrowError(TeoExeption::createNewInstance("Connector.login()", "Connection was not been opened."));
    }
    else {
	size_t snd(This->login(*Nan::Utf8String(info[0])));
	if(snd == (size_t)-1) {
	    TeoErrnoExeption::createNewInstance(errno, "Connector.login()");
	}
	else {
...
NAN_MODULE_INIT(Connector::Init) {
    ...
    auto enumm(Nan::New("emumm").ToLocalChecked());
    Nan::SetAccessor(ctorInst, enumm, Enum, nullptr);
