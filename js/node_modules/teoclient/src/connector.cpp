...
Connector::Worker::Worker(Callback *callback, const std::string& ip, int port) 
...
// Execute acync
void Connector::Worker::Execute () {
    connector_ = teoLNullConnect(ip_.c_str(), port_);
    if(connector_ == nullptr || connector_->fd <= 0) {
	terrno_ = errno; // Save errno
	if(connector_)
    	    teoLNullDisconnect(connector_);
	connector_ = nullptr;
    }
...
void Connector::Worker::HandleOKCallback () {
    Nan:: HandleScope scope;
    if(connector_) {
	auto obj(Nan::New<v8::Function>(constructor)->NewInstance(0, {}));
	auto This(Nan::ObjectWrap::Unwrap<Connector>(obj));
	This->attach_connection(ip_, port_, connector_);
	Local<Value> argv[] = {obj, Nan::New<Integer>(0)};
	callback->Call(2, argv);
    }
    else {
	Local<Value> argv[] = {Null(), Nan::New<Integer>(terrno_)};
	callback->Call(2, argv);
    }
}
