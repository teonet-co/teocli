...
NAN_METHOD(TeoExeption::New) {
    ...
    auto object = new TeoExeption(info[0]->IntegerValue(), *Nan::Utf8String(info[1]), *Nan::Utf8String(info[2]));
...
NAN_GETTER(TeoExeption::ErrnoGet) {
    Nan::HandleScope scope;
...
NAN_SETTER(TeoExeption::ErrnoSet) {
    Nan::HandleScope scope;
    auto object = Nan::ObjectWrap::Unwrap<TeoExeption>(info.Holder());
...
Local<Value> TeoExeption::createNewInstance(int errn, const std::string& call_name) {
    int arcv (3);
    char buf [4096];
    ...
    return Nan::New<v8::Function>(constructor)->NewInstance(arcv, argv);
}
